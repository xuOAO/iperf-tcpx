...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
...
#ifdef HAVE_LIBTCPX
        {"tcpx-rxdev", required_argument, NULL, OPT_TCPX_RXDEV},
        {"tcpx-txdev", required_argument, NULL, OPT_TCPX_TXDEV},
        {"tcpx-rxmem", required_argument, NULL, OPT_TCPX_RXMEM},
        {"tcpx-txmem", required_argument, NULL, OPT_TCPX_TXMEM},
        {"tcpx-rxmemsz", required_argument, NULL, OPT_TCPX_RXMEMSZ},
        {"tcpx-txmemsz", required_argument, NULL, OPT_TCPX_TXMEMSZ},
        {"start-queue", required_argument, NULL, OPT_START_QUEUE},
        {"num-queue", required_argument, NULL, OPT_NUM_QUEUE},
	...
	case OPT_USE_PKCS1_PADDING:
...
#ifdef HAVE_LIBTCPX
	    case OPT_TCPX_RXDEV:
		test->tcpx_rxdev = strdup(optarg);
		break;
	    case OPT_TCPX_TXDEV:
		test->tcpx_txdev = strdup(optarg);
		break;
	    case OPT_TCPX_RXMEM:
		if (strcmp(optarg, "host") == 0) {
		    test->tcpx_rxmem_type = TCPX_MEM_TYPE_HOST;
		} else if (strcmp(optarg, "cuda") == 0) {
		    test->tcpx_rxmem_type = TCPX_MEM_TYPE_CUDA;
		} else {
		    i_errno = IEBADFORMAT;
		    return -1;
		}
	    ...
	    case OPT_TCPX_TXMEM:
		if (strcmp(optarg, "host") == 0) {
		    test->tcpx_txmem_type = TCPX_MEM_TYPE_HOST;
		} else if (strcmp(optarg, "cuda") == 0) {
		    test->tcpx_txmem_type = TCPX_MEM_TYPE_CUDA;
		} else {
		    i_errno = IEBADFORMAT;
		    return -1;
		}
		break;
	    case OPT_TCPX_RXMEMSZ:
		test->tcpx_rxmem_size = (size_t)unit_atoi(optarg);
		if (test->tcpx_rxmem_size <= 0) {
		    i_errno = IEBADFORMAT;
		    return -1;
		}
		break;
	    case OPT_TCPX_TXMEMSZ:
		test->tcpx_txmem_size = (size_t)unit_atoi(optarg);
		if (test->tcpx_txmem_size <= 0) {
		    i_errno = IEBADFORMAT;
		    return -1;
		}
		break;
	    case OPT_START_QUEUE:
		test->start_queue = atoi(optarg);
		if (test->start_queue < 0) {
		    i_errno = IEBADPORT;
		    return -1;
		}
		break;
	    case OPT_NUM_QUEUE:
		test->num_queue = atoi(optarg);
		if (test->num_queue <= 0) {
		    i_errno = IEBADPORT;
		    return -1;
		}
		...
		if (strcmp(optarg, "NONE") == 0 || strcmp(optarg, "0") == 0) {
		    test->tcpx_debug_level = 0;  // TCPX_DEBUG_NONE
		} else if (strcmp(optarg, "ERROR") == 0 || strcmp(optarg, "1") == 0) {
		    test->tcpx_debug_level = 1;  // TCPX_DEBUG_ERROR
		} else if (strcmp(optarg, "WARN") == 0 || strcmp(optarg, "2") == 0) {
		    test->tcpx_debug_level = 2;  // TCPX_DEBUG_WARN
		} else if (strcmp(optarg, "INFO") == 0 || strcmp(optarg, "3") == 0) {
		    test->tcpx_debug_level = 3;  // TCPX_DEBUG_INFO
		} else if (strcmp(optarg, "VERBOSE") == 0 || strcmp(optarg, "4") == 0) {
		    test->tcpx_debug_level = 4;  // TCPX_DEBUG_VERBOSE
		} else if (strcmp(optarg, "ALL") == 0 || strcmp(optarg, "5") == 0) {
		    test->tcpx_debug_level = 5;  // TCPX_DEBUG_ALL
		} else {
		    test->tcpx_debug_level = atoi(optarg);
		    if (test->tcpx_debug_level < 0 || test->tcpx_debug_level > 5) {
			i_errno = IEBADFORMAT;
			return -1;
		    }
		}
    ...
    */
    ...
    if (test->tcpx_rxdev) {
        ...
        // 创建 RX NIC 上下文
        int* queue_array = malloc(test->num_queue * sizeof(int));
        if (!queue_array) {
            i_errno = IENEWTEST;
            return -1;
        }
        for (int i = 0; i < test->num_queue; i++) {
            queue_array[i] = test->start_queue + i;
        }
        ...
        iperf_printf(test, "TCPX 已启用：rx设备=%s, RX队列=%d-%d\n", 
                     test->tcpx_rxdev, test->start_queue, 
                     test->start_queue + test->num_queue - 1);
    }
    ...
    if (test->tcpx_txdev) {
        ...
        iperf_printf(test, "TCPX 已启用：tx设备=%s\n", test->tcpx_txdev);
    }
#endif
...
#ifdef HAVE_LIBTCPX
    // 初始化 TCPX 相关字段的默认值
    testp->tcpx_rxdev = NULL;
    testp->tcpx_txdev = NULL;
    testp->rxnic_ctx = NULL;
    testp->txnic_ctx = NULL;
    testp->server_ctx = NULL;
    testp->tcpx_rxmem_type = TCPX_MEM_TYPE_HOST;  // 默认使用host内存
    testp->tcpx_txmem_type = TCPX_MEM_TYPE_HOST;  // 默认使用host内存
    testp->tcpx_rxmem_size = 16000 * 4096;        // 默认RX内存大小：64MB
    testp->tcpx_txmem_size = 16000 * 4096;        // 默认TX内存大小：64MB
    testp->start_queue = 1;                       // 默认起始队列号
    testp->num_queue = 16;                        // 默认队列数量
